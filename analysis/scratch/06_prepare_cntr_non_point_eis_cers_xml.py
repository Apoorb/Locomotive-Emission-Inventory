...
path_txled = os.path.join(PATH_PROCESSED, "txled_factors_by_county_prc.csv")
...
temp_fips_scc = cntr_emisquant_nonpoint_2020[
    ["StateAndCountyFIPSCode", "SourceClassificationCode"]
].drop_duplicates()
txled_df = pd.read_csv(path_txled)
txled_df_fil = (
    txled_df.loc[lambda df: df.txled_fac != 1]
    .assign(
        ControlMeasureCode=30,
        PercentControlMeasureReductionEfficiency=lambda df: (1 - df.txled_fac) * 100,
    )
    .rename(
        columns={
            "FIPS_ST_CNTY_CD": "StateAndCountyFIPSCode",
...
txled_df_fil_cntr_measure = txled_df_fil.filter(items=cntr_measure_cols)

txled_df_fil_cntr_pol = txled_df_fil.filter(items=cntr_pol_cols)
...
cntr_emisquant_nonpoint_emisssions_a = cntr_emisquant_nonpoint_2020.assign(
...
cntr_emisquant_nonpoint_emisssions_o3d = (
    cntr_emisquant_nonpoint_2020.loc[
...
uncntr_emisquant_nonpoint_emisssions = pd.concat(
    [cntr_emisquant_nonpoint_emisssions_a, cntr_emisquant_nonpoint_emisssions_o3d]
