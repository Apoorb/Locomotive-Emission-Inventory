...
fuel_consump_from_data = ("BNSF", "KCS", "UP", "ANR", "AWRR", "CTXR", "MCSA",
                          "PTRA", "TCT", "TN", "TXGN", "TXOR", "TXPF")
map_rrgrp = {
...
txrail_2020_preprs_agg_st = txrail_2020_preprs.groupby(
    ["carrier", "net"]
).agg(
    totnetmiles=("miles", "sum"),
)

txrail_2020_preprs_agg_st_fuel = (
    txrail_2020_preprs_agg_st
    .merge(fuel_collected_df, on=["carrier", "friylab"])
)


txrail_2020_preprs_agg_st_fuel_rate = (
    txrail_2020_preprs_agg_st_fuel
    .groupby("friylab")["totnetmiles", "fuel_consump"].sum().reset_index()
