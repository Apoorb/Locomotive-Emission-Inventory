...
from locoerlt.utilis import (
    PATH_RAW, PATH_INTERIM, PATH_PROCESSED, get_snake_case_dict)
...
path_narl_yards = glob.glob(
    os.path.join(PATH_INTERIM, "fuelconsump_2019_tx_*.csv")
)[0]


tx_counties = (
    pd.read_csv(path_county_nm)
)

tx_counties_fil = (
    tx_counties
    .rename(columns=get_snake_case_dict(tx_counties))
...
ertac_yards_2020 = (
    ...
    .drop_duplicates([
        ...
        "eis_facility_id",
        "yardname_v1"])
    .assign(yardname=lambda df: df.yardname_v1.str.lower(),
            cnty_nm=lambda df: df.county_name,
            data="yes"
            )
    .rename(columns={"site_latitude": "lat", "site_longitude": "long"})
    .filter(items=["stcntyfips", "eis_facility_id", "yardname",
                   "data", "lat", "long"])
...
narl_yards_2020 = (
    ...
    .assign(
        yardname=lambda df: df.yardname.str.lower(),
        data="yes"
    )
    .reset_index(drop=True)
...
narl_ertac_yards_2020_common = (
    narl_ertac_yards_2020
    .loc[lambda df: (df["Found in ERTAC?"] == "yes")
                    & (df["Found in NARL?"] == "yes")]
)
