...
def print_xml_lines(tree_elem: xml.etree.ElementTree.Element, max_lines=10
                    ) -> None:
    line_no = 1
    for line in ET.tostring(tree_elem, encoding='utf8').decode(
            'utf8').split("\n")[0: max_lines]:
...
def set_creation_datetime(templ_root_header_: xml.etree.ElementTree.Element
                          ) -> None:
...
def set_data_cat_prop(templ_root_header_: xml.etree.ElementTree.Element,
                       doc_title=""
                       ) -> None:
...
def modify_template_header(templ_root_: xml.etree.ElementTree.Element) -> xml.etree.ElementTree.Element:
...
for line in ET.tostring(templ_root_payload_cers, encoding='utf8').decode(
        'utf8')[0:1000].split("\n"):
    print(line)
