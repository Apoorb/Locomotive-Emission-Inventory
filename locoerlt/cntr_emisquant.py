...
def get_deri_quantity_red(
    ...
    assert (
        np.round(deri_loco_nox_red_yr_prcd.nox_red_tons_per_yr.sum(), 4) == 51202.7567
    ), (
    ...
    deri_loco_nox_red_yr_prcd_emis_quant_region = (
        deri_loco_nox_red_yr_prcd.merge(
            emis_quant_region_deri, on=["region", "year"], how="inner"
        )
        .assign(
        ...
        .filter(
            items=[
                ...
                "nox_red_tons_per_yr",
                "pollutant",
                "em_quant_yard_region_tons",
                "em_quant_yard_region_tons_cntr",
    ...
    deri_loco_nox_red_yr_prcd_emis_quant_county = (
        deri_loco_nox_red_yr_prcd_emis_quant_region.merge(deri_regions, on=["region"])
        .filter(items=["counties", "year", "pollutant", "deri_regional_em_reduc_fac"])
        .assign(tp_county_nm=lambda df: df.counties.str.lower().str.strip())
    )
    return {
        "deri_loco_nox_red_yr_prcd_emis_quant_region": deri_loco_nox_red_yr_prcd_emis_quant_region,
        "deri_loco_nox_red_yr_prcd_emis_quant_county": deri_loco_nox_red_yr_prcd_emis_quant_county,
    }
...
def get_deri_controlled_fac(
    controlled_emis_quant_txled_: pd.DataFrame, deri_county_fac_: pd.DataFrame
) -> pd.DataFrame:
    """
    ...
    """
    controlled_emis_quant_txled_deri = controlled_emis_quant_txled_.merge(
        (
            deri_county_fac.filter(
                items=[
                    "tp_county_nm",
                    "year",
                    "pollutant",
...
if __name__ == "__main__":
    ...
    deri_county_fac = deri_red_fac_dict["deri_loco_nox_red_yr_prcd_emis_quant_county"]
