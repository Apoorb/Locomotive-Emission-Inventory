...
def get_epa_eis_facility_unit_prc_identifiers(
    path_epa_eis_info_: str,
    path_cntr_emisquant_:str
):
    """
    ...
    unique_yards_ertac_2017 = (
        ...
        .loc[lambda df: (df.scc_description_level_4 == "Yard Locomotives")
                            & (df.year == 2020) & (df.pollutant == "CO")
        ]
        .drop_duplicates(
            ["stcntyfips", "eis_facility_id", "yardname_v1"]
        )
        .filter(
            items=["stcntyfips", "eis_facility_id", "yardname_v1"]
        )
        .assign(
            ertac_yards=1,
        )
    ...
    assert (
        epa_yard_info_fil.groupby("eis_facility_id")["county"].count().max()
        == 1), "eis_facility_id is not unique."

    unique_yards_ertac_2017_epa_unit_prc_identifiers = (
        unique_yards_ertac_2017
        .merge(
            epa_yard_info_fil,
...
def set_yard_uncntr_cntr_xml(
    ...
    uncntr_cntr_emisquant_yards_2020 = (
        ...
        .filter(
            items=["stcntyfips", "county_name", "scc_description_level_4",
                   "eis_facility_id", "eis_unit_id", "eis_process_id",
                   "yardname_v1", "pollutant", "site_latitude",
                   "site_longitude", emis_quant_ton_col])
        .rename(columns={
...
def set_facilitysite_yard_xml(
    uncntr_cntr_emisquant_yards_2020_,
    conn_,
    cursor_
):
    ...
    cursor_.executemany(sql,
                        uncntr_cntr_emisquant_yards_fac_info.itertuples(
                            index=False)
                        )
    conn_.commit()
...
def set_emissiosn_yard_xml(
    ...
    uncntr_cntr_emisquant_yards_emisssions = (
        pd.concat(
            [uncntr_cntr_emisquant_yards_2020_emisssions_a,
             uncntr_cntr_emisquant_yards_2020_emisssions_o3d
             ]
    ...
    cursor_.executemany(sql, uncntr_cntr_emisquant_yards_emisssions.itertuples(
        index=False))
    conn_.commit()
...
if __name__ == "__main__":
    ...
    epa_eis_facility_unit_prc_identifiers = (
        get_epa_eis_facility_unit_prc_identifiers(
            path_epa_eis_info_=path_epa_eis_info,
