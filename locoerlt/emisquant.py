...
if __name__ == "__main__":
    ...
    county_df_fil = (
        county_df
        .filter(items=["CNTY_NM", "FIPS_ST_CNTY_CD"])
        .rename(columns={"CNTY_NM": "county_name", "FIPS_ST_CNTY_CD":
            "stcntyfips"})
    )
    ...
    fuel_consump_prj = (
        fuel_consump
        .filter(items=[
            'stcntyfips', 'carrier', 'friylab', 'rr_netgrp', 'rr_group',
            'netfuelconsump'
        ])
        ...
        .assign(fuelconsump=lambda df:
                df.aeo_with_covid_travel_freight_proj_fac
                * df.netfuelconsump_2019,
                year=lambda df:df.year.astype(int))
    )

    assert ~ fuel_consump_prj.isna().any().any(), "Check dataframe for nan"
    assert (fuel_consump_prj.loc[lambda df: df.year==2019]
            .eval("netfuelconsump_2019 == fuelconsump").all()), (
    ...
    fuel_consump_prj_by_cnty = (
        fuel_consump_prj
        .groupby(['year', 'stcntyfips', 'carrier', 'friylab', 'rr_netgrp',
                  'rr_group']
    ...
    xwalk_ssc_desc_4_rr_grp_friy = StringIO("""scc_description_level_4,rr_group,friylab
    ...
    """)
    xwalk_ssc_desc_4_rr_grp_friy_df = (
        pd.read_csv(xwalk_ssc_desc_4_rr_grp_friy, sep=",")
        .assign(scc_description_level_4=lambda df:
                df.scc_description_level_4.str.strip())
    )

    fuel_consump_prj_by_cnty_scc = (
        fuel_consump_prj_by_cnty
        .merge(xwalk_ssc_desc_4_rr_grp_friy_df,
               on=["rr_group", "friylab"],
               how="outer")
    )
    ...
    emis_quant = (
        ...
        .drop(
            columns=['anals_yr', 'em_units', 'friylab']
        )
    )
