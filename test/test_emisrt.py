...
def get_nox_pm10_pm25_voc_epa_em_fac():
    ...
    nox_pm10_hc_df_impute = (
        ...
        .filter(
            items=["scc_description_level_4", "pollutant", "year", "em_fac"])
    )
    voc_df = (
        nox_pm10_hc_df_impute.loc[nox_pm10_hc_df_impute.pollutant == "HC"]
        .assign(
            pollutant="VOC",
    ...
    pm25_df = (
        nox_pm10_hc_df_impute.loc[nox_pm10_hc_df_impute.pollutant == "PM10-PRI"]
        .assign(
            pollutant="PM25-PRI",
            em_fac=lambda df: df.em_fac * 0.97
        )
    )

    nox_pm10_pm25_voc_df = (
        pd.concat([nox_pm10_hc_df_impute, voc_df, pm25_df])
...
def test_so2_fac(get_out_emis_fac):
    ...
    assert (all(so2_emis_fac_2011.em_fac == so2_2011_500_ppm_s)
...
def test_nox_pm10_pm25_voc_epa_em_fac_2011_40(get_out_emis_fac,
...
def test_nox_pm10_pm25_voc_epa_em_fac_2040_50(get_out_emis_fac,
                                              get_nox_pm10_pm25_voc_epa_em_fac):
    ...
    testdf["em_fac_emisfacout"] = (
        testdf.groupby(["pollutant", "scc_description_level_4"])
        .em_fac_emisfacout.ffill())

    assert np.allclose(testdf.em_fac_epa2009,
                       testdf.em_fac_emisfacout)
...
def test_hap_speciation(get_out_emis_fac, hap_speciation):
    pm25_voc = (
        get_out_emis_fac
        .loc[lambda df: (df.pollutant.isin(["PM25-PRI", "VOC"]))]
        .filter(items=["anals_yr", "pollutant", "scc_description_level_4",
                    "em_fac"])
        .reset_index(drop=True)
        .rename(columns={"em_fac": "em_fac_input",
                         "pollutant": "input_pollutant_code"})
        )

    hap_em_fac = (
        hap_speciation
        .merge(pm25_voc, on=["input_pollutant_code", "scc_description_level_4"])
        .assign(em_fac=lambda df: df.multiplication_factor * df.em_fac_input,
                output_pollutant_code=lambda df:
                    df.output_pollutant_code.astype(str))
        .filter(
            items=["anals_yr", "scc_description_level_4",
                   "output_pollutant_code", "multiplication_factor",
                   "em_fac_input", "em_fac"]
        )
    )
    ...
    test_df = (
        ...
        .merge(hap_em_fac,
               ...
               right_on=["anals_yr", "scc_description_level_4",
...
def test_hap_speciation(get_out_emis_fac):
    pm10_fac = (
        get_out_emis_fac
        .loc[lambda df: (df.pollutant.isin(["PM10-PRI"]))]
        .filter(items=["anals_yr", "pollutant", "scc_description_level_4",
                       "em_fac"])
        .reset_index(drop=True)
        .rename(columns={"em_fac": "em_fac_input",
