...
path_cntr_xml_epa = os.path.join(PATH_PROCESSED, "eis_stagging_tables",
                                 "nonpoint_bridgetool_cntr.xml")
path_uncntr_xml_txaer = os.path.join(PATH_PROCESSED, "uncntr_cers_tx.xml")
path_uncntr_xml_epa = os.path.join(PATH_PROCESSED, "eis_stagging_tables",
                                 "nonpoint_bridgetool_uncntr.xml")
...
def get_state_fuel_consmp_yard_xml(path_xml):
    ...
    cntr_yard_xml_df = \
        test_yard_cers_xml_epa.get_annual_o3d_emissions_df_from_xml(
    ...
    cntr_yard_xml_df_fil = (
        cntr_yard_xml_df
            .loc[lambda df: df.pollutant == "CO2"]
            .assign(
            stcntyfips=lambda df: df.stcntyfips.astype(int),
            em_quant_ton=lambda df:
            df.em_quant_ton.astype(float),
            fuel_consump=lambda df: (df.em_quant_ton * us_ton_to_grams
                                     / co2_emis_fac),
            year=2020,
            ssc=2285002010
        )
