...
def test_controlled_emis_quant(get_emis_quant_agg,
                               get_cntr_emisquant,
                               get_txled_counties):
    ...
    test_emisquant_nox_txled = test_emisquant.loc[lambda df: (
            (df.pollutant == "NOX") & (df.county_name_pre.str.lower()
        .isin(txled_counties))
        )
    ]
    ...
    test_emisquant_non_txled = test_emisquant.loc[lambda df: ~(
         (df.pollutant == "NOX") & (df.county_name_pre.str.lower()
                                    .isin(txled_counties))
        )
    ]
    ...
    assert (are_all_nox_em_quant_in_txled_counties_adjusted
            and are_all_non_txled_em_quant_unadjusted)


@pytest.mark.parametrize("analysis_year_deri_benefits", [27206.2667])
def test_uncontrolled_emis_quant(get_emis_quant_agg,
    ...
    test_emisquant_nox = test_emisquant.loc[lambda df: (
            (df.pollutant == "NOX")
        )
    ]
    ...
    total_deri_benefits_in_analysis_year = (
        ...
        - test_emisquant_nox.em_quant_pre / us_ton_to_grams).sum()
