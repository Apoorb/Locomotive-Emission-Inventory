...
def test_county_control_tot_cls1(
    get_emis_quant, get_fuel_consump, get_county_cls1_prop_input
):
    fuel_df_cls1 = get_fuel_consump.loc[
        lambda df: (df.carrier.isin(["BNSF", "UP", "KCS"])) & (df.friylab == "Fcat")
    ]

    fuel_df_cls1_agg = (
        fuel_df_cls1.groupby(["carrier", "friylab", "stcntyfips"])
        .agg(
    ...
    fuel_df_cls1_agg["st_fuel_consmp_computed"] = fuel_df_cls1_agg.groupby(
        ["carrier", "friylab"]
    ...
    test = co2_nh3_co_fil.merge(
        fuel_df_cls1_agg, on=["carrier", "friylab", "stcntyfips"], how="right"
    )
    test.groupby("carrier").sum()
    test_1 = test.loc[
        ~np.isclose(test.cnty_cls1_fuel_consmp_1, test.county_fuel_consmp_by_yr)
    ]

    co2_nh3_co["county_pct"] = (
        co2_nh3_co["county_fuel_consmp_by_yr"] / co2_nh3_co["st_fuel_consmp_by_yr"]
    )
    cnt_pct = get_county_cls1_prop_input.rename(columns={"FIPS": "stcntyfips"})
    co2_nh3_co_test = co2_nh3_co.merge(cnt_pct, on="stcntyfips")
