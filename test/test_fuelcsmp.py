...
from locoerlt.fuelcsmp import (
    get_fuel_consmp_by_cnty_carrier, preprc_link, preprc_fuelusg)
...
def get_county_cls1_prop_py_cd(get_cls1_cls3_comm_passg_py_df):
    county_cls1_prop_py_cd = (
        ...
        .loc[lambda df: df.rr_group == "Class I"]
...
def get_cls3_comm_passg_py_df(get_cls1_cls3_comm_passg_py_df):
    txrail_milemx_cls_3_comut_pasng_19_comp = (
        get_cls1_cls3_comm_passg_py_df
        .loc[lambda df: df.rr_group != "Class I"]
        .rename(columns={"st_fuel_consmp": "totfuelconsump",
...
def get_cls3_comm_passg_sql_df():
    ...
    fuelconsump_sql_mar5_comp = (
        fuelconsump_sql_mar5.rename(
            columns={
                col: inflection.underscore(col) for col in
                fuelconsump_sql_mar5.columns
            }
        )
        .rename(
            columns={
...
def test_all_carriers_in_natrail(
        get_cls1_cls3_comm_passg_py_df, get_carrier_df):
    carriers_py_cd = (
        get_cls1_cls3_comm_passg_py_df
        .groupby(['carrier', 'rr_group'])["link_fuel_consmp"].sum()
        .reset_index()
...
def test_milemx_cls1_grp_cnty_1_oth_carrier_grp_st_1(
        get_cls1_cls3_comm_passg_py_df):
    is_cls1_milemx_cnty_1 = np.allclose(
        (get_cls1_cls3_comm_passg_py_df
         .loc[lambda df: df.rr_group == "Class I"]
         .groupby(['carrier', 'friylab', "stcntyfips"]).milemx.sum().values),
        1
    )
    is_notcls1_milemx_st_1 = np.allclose(
        (get_cls1_cls3_comm_passg_py_df
         .loc[lambda df: df.rr_group != "Class I"]
         .groupby(['carrier', 'friylab']).milemx.sum().values),
        1
    )
    assert is_cls1_milemx_cnty_1 & is_notcls1_milemx_st_1


def test_county_cls1_prop_cnt_tots(get_county_cls1_prop_py_cd,
                                   get_county_cls1_prop_input):
    county_cls1_prop_py_cd_prcsd=(
        get_county_cls1_prop_py_cd
        .groupby("stcntyfips").county_pct.mean().reset_index()
        .rename(columns={"stcntyfips": "FIPS", "county_pct": "CountyPCT"})
        .assign(FIPS=lambda df: df.FIPS.astype("int64"))
     )
    pd.testing.assert_frame_equal(get_county_cls1_prop_input,
                                  county_cls1_prop_py_cd_prcsd)
...
def test_cls1_fuel_consump_tots_by_cnty_by_st(
    ...
    assert (is_cls1_sum_link_fuel_eq_cnty_fuel
...
def test_notcls1_fuel_consump_tots_by_by_st(
    ...
    cls3_pass_comut_st_totals = (
        get_cls3_comm_passg_py_df
        .rename(columns={"totfuelconsump": "st_fuel_consmp",
    ...
    assert (is_notcls1_sum_link_fuel_eq_st_fuel
            & is_notcls1_input_fuel_eq_st_fuel)